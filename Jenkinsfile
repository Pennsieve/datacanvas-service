...
node("executor") {
    ...
    if (env.BRANCH_NAME == 'main') {
        ...
        stage("Docker") {
            ...
            sh "docker tag pennsieve/datacanvas-service:latest pennsieve/datacanvas-service:$imageTag"
            sh "docker push pennsieve/datacanvas-service:latest"
            sh "docker push pennsieve/datacanvas-service:$imageTag"
        }

        stage("Deploy") {
            build job: "service-deploy/pennsieve-non-prod/us-east-1/dev-vpc-use1/dev/datacanvas-service",
            parameters: [
